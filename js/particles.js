...
(function () {
  ...
  function makeSprite(kind) {
    ...
    if (kind === 0) {                                   /* golden & violet dust */
      var gd = g.createRadialGradient(h, h, 0, h, h, h);
      gd.addColorStop(0, 'rgba(252,242,255,1)');
      gd.addColorStop(0.22, 'rgba(212,168,230,0.78)');
      gd.addColorStop(0.58, 'rgba(154,109,176,0.25)');
      gd.addColorStop(1, 'rgba(154,109,176,0)');
      g.fillStyle = gd;
      g.beginPath(); g.arc(h, h, h, 0, TAU); g.fill();

    } else if (kind === 1) {                            /* glowing star — champagne & soft violet */
      var gs = g.createRadialGradient(h, h, 0, h, h, h * 0.34);
      gs.addColorStop(0, 'rgba(255,250,240,1)');
      gs.addColorStop(0.5, 'rgba(229,200,133,0.65)');
      gs.addColorStop(1, 'rgba(197,160,89,0)');
      g.fillStyle = gs;
      g.beginPath(); g.arc(h, h, h * 0.34, 0, TAU); g.fill();

      var gsp = g.createRadialGradient(h, h, 0, h, h, h);
      gsp.addColorStop(0, 'rgba(255,248,235,0.95)');
      gsp.addColorStop(0.35, 'rgba(229,200,133,0.4)');
      gsp.addColorStop(1, 'rgba(197,160,89,0)');
      g.fillStyle = gsp;
    ...
    } else {                                            /* soft lavender bubble */
      var gb = g.createRadialGradient(h, h * 0.8, 0, h, h, h * 0.72);
      gb.addColorStop(0, 'rgba(215,185,235,0.32)');
      gb.addColorStop(0.72, 'rgba(137,75,161,0.08)');
      gb.addColorStop(1, 'rgba(137,75,161,0)');
      g.fillStyle = gb;
      g.beginPath(); g.arc(h, h, h * 0.72, 0, TAU); g.fill();

      g.strokeStyle = 'rgba(205,170,225,0.42)';
      g.lineWidth = 1.6;
